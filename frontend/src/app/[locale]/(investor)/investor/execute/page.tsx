...
const schema = z.object({
  ...
  reasonForInvesting: z.string().min(20, "Explain your reason for investing in at least 20 characters"),
  investmentBudget: z.number().min(1, "Enter your investment budget"),
  dreamOfSuccess: z.string().min(20, "Describe your vision of success in at least 20 characters"),
...
const industries = [
  "Fintech", "Healthtech", "Agritech", "Edtech", "Logistics",
  "E-commerce", "SaaS", "Clean Energy", "Real Estate", "Media & Entertainment", "Other",
];
...
export default function InvestorExecutePage() {
  ...
  const { user } = useAuth();
  const t = useTranslations("investor.execute");
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-[var(--color-neutral-500)] mt-1">{t("subtitle")}</p>
      </div>
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              />
              {errors.reasonForInvesting && <p className="text-xs text-red-500">{errors.reasonForInvesting.message}</p>}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input
                  type="number"
                  placeholder="500000"
                  className={`pl-7 ${errors.investmentBudget ? "border-red-500" : ""}`}
                  {...register("investmentBudget", { valueAsNumber: true })}
                />
              </div>
              {errors.investmentBudget && <p className="text-xs text-red-500">{errors.investmentBudget.message}</p>}
            </div>

            <div className="space-y-1.5">
              <Label>{t("dreamLabel")}</Label>
              <Textarea
              ...
              />
              {errors.dreamOfSuccess && <p className="text-xs text-red-500">{errors.dreamOfSuccess.message}</p>}
            </div>

            <div className="space-y-1.5">
              <Label>
                {t("criteriaLabel")}{" "}
                <span className="text-[var(--color-neutral-400)] font-normal">{t("criteriaOptional")}</span>
              </Label>
              <Textarea
            ...
            <Button type="submit" className="w-full gap-2" disabled={isSubmitting}>
              {isSubmitting ? (
                <><Loader2 className="h-4 w-4 animate-spin" />{t("startingSession")}</>
              ) : (
