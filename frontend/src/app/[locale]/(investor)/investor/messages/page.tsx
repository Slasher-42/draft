  ...
  const { data: collaborators = [], isLoading } = useQuery<Collaborator[]>({
    ...
    queryFn: async () => {
      ...
      await Promise.allSettled(
        matches.map(async (m) => {
          if (seen.has(m.startupUserId)) return;
          seen.add(m.startupUserId);
          try {
            const u = await userService.getById(m.startupUserId);
            let sp = null;
            try { sp = await userService.getStartupProfile(m.startupUserId); } catch {}
            const name = sp?.companyName ?? u?.fullName ?? `Startup #${m.startupUserId}`;
            list.push({
              userId: m.startupUserId,
              fullName: name,
          ...
          {isLoading ? (
            ...
            filtered.map((c) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-[var(--color-neutral-50)] ${selected?.userId === c.userId ? "bg-[var(--color-primary-50)] border-r-2 border-[var(--color-primary)]" : ""}`}
              >
          ...
          <div className="p-4 border-t border-[var(--color-border)] bg-[var(--color-card)]">
            <div className="flex items-end gap-2 bg-[var(--color-neutral-50)] border border-[var(--color-border)] rounded-2xl px-4 py-2">
              ...
              <button
                ...
                style={{ background: input.trim() ? "linear-gradient(135deg,#0B4A8B,#2F72A5)" : "var(--color-neutral-200)" }}
              >
                {sending
