...
export default function StartupExecutionsPage() {
  const { executions, loading } = useStartupExecutions();
  ...
  return (
    ...
    >
      <DashboardHeader
        title="My Executions"
        subtitle="Track the status of all your startup executions"
        actions={
      ...
      {executions.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 gap-4">
            <p className="text-surface-muted text-sm">You have not submitted any executions yet.</p>
            <Link href="/startup/execute">
                ...
                <CardContent className="flex items-center justify-between py-5">
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="text-surface-muted text-xs">
                      {execution.targetCompanySize} · ${execution.fundingNeeded.toLocaleString()} needed · Submitted {formatDate(execution.createdAt)}
                    </p>
                  ...
                  <Link href={`/startup/executions/${execution.id}`}>
