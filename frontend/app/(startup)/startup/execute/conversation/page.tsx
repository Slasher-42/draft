      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-4">
        <AnimatePresence initial={false}>
          {messages.map((msg) => (
            ...
            >
              {msg.role === 'aria' && (
                <div className="h-8 w-8 rounded-full bg-brand-500/20 border border-brand-500/30 flex items-center justify-center mr-3 shrink-0 mt-1">
                  <span className="text-brand-400 font-bold text-xs">A</span>
                </div>
              )}
              <div
        ...
        {isLoading && (
          ...
          >
            <div className="h-8 w-8 rounded-full bg-brand-500/20 border border-brand-500/30 flex items-center justify-center mr-3 shrink-0">
              <span className="text-brand-400 font-bold text-xs">A</span>
            </div>
        ...
        {phase === 'asking_considerations' && !isLoading && (
          ...
          >
            ...
            <textarea
              ...
              className="w-full rounded-lg border border-surface-border bg-surface text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400 resize-none"
            />
            <Button
              variant="primary"
              size="sm"
              className="self-end"
              loading={isLoading}
              onClick={() => finishConversation(executionId, 'STARTUP')}
            >
              Submit & Finish
            </Button>
          </motion.div>
        ...
        {phase === 'done' && (
          ...
          >
            ...
            <Button variant="primary" onClick={() => router.push('/startup/executions')}>
              View My Executions
            </Button>
          </motion.div>
        )}
        ...
        <div className="border-t border-surface-border px-6 py-4 bg-surface-subtle">
          <div className="flex items-end gap-3">
            <textarea
              ...
              disabled={isLoading}
              className="flex-1 rounded-xl border border-surface-border bg-surface-card text-white px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400 resize-none disabled:opacity-50"
            />
            <Button
              variant="primary"
              size="sm"
